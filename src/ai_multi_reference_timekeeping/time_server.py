...
class SensorFrame:
    ...
    Each field is optional and represents a specific sensor-derived signal.
    The inference models treat missing values as neutral (no influence).
    """
...
class SensorAggregator:
    ...
    def __init__(
        ...
        *inputs: SensorInput,
        ...
        # THREAD-SAFETY: _partitions is shared across samples; guarded by the
        # single-threaded sampling loop in this aggregator.
...
class LightweightInferenceModel:
    ...
    This model prioritizes transparency over sophistication. It applies
    penalty multipliers when environmental conditions indicate instability.
    """
...
class LinearInferenceModel:
    ...
    A linear score is mapped through a logistic curve to keep scaling bounded.
    """
...
class MlVarianceModel:
    ...
    Updates are bounded (for safety) and based on residual statistics so that
    anomalous measurements do not poison the learned bias.
    """
...
class SlewDriftDetector:
    ...
    Slew is a coarse difference over the window. Drift uses a least-squares fit.
    """
...
class I2CEnvironmentalSensor:
    ...
    This is a thin wrapper over a provided callable so that hardware drivers
    can be injected and tested without binding to a specific I2C library.
    """
...
class AudioFeatureSensor:
    ...
    The Goertzel algorithm provides narrowband energy estimates for each target
    frequency. This keeps signal processing lightweight and explainable.
    """
...
class TimeServer:
    ...
    The time server is intentionally synchronous. A coarse lock ensures that
    multi-threaded callers do not interleave state updates.
    """
    ...
    def __init__(
        ...
        self._clock = clock
        # THREAD-SAFETY: _references is immutable after init.
        self._references = list(references)
        # THREAD-SAFETY: _sensors is not thread-safe; access is serialized by _lock.
        ...
        # THREAD-SAFETY: _drift_detector is mutated during step; use _lock.
        ...
        # THREAD-SAFETY: _security tracks last measurements; use _lock.
        self._security = security_monitor or SecurityMonitor(safety_case=self._safety)
        self._logger = logger or logging.getLogger(__name__)
        # THREAD-SAFETY: _lock serializes step_with_context across threads.
    ...
    def state(self) -> ClockState:
        """Return the current virtual clock state estimate."""
...
class RateLimiter:
    ...
    This is intentionally deterministic and easy to audit. Token buckets are
    not used to avoid hidden state or ambiguity.
    """
...
class SecurityMonitor:
    ...
    Alerts are recorded for hazard tracking and used for operator visibility.
    """
