...
**Model architecture**  
The “AI-assisted” component is intentionally lightweight: it maps environment/context signals to measurement variance scaling rather than altering raw timestamps. The system currently supports:
...
**Training data**  
The repository does not yet include a large real-world dataset. The models can be trained on synthetic or logged datasets of `(sensor_frame, reference_offsets)`; the `configs/gnss_dropout.yaml` experiment is the canonical synthetic setup.

**Objective function**  
The objective is to minimize stability metrics (TDEV/MTIE) and reduce drift during reference loss, indirectly by adjusting reference variance.
