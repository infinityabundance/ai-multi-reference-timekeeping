...
class SensorAggregator:
    ...
    def __init__(self, *inputs: SensorInput, validator: "SensorValidator" | None = None) -> None:
        self._inputs = inputs
        self._validator = validator or SensorValidator()
...
class TimeServer:
    ...
    def __init__(
        ...
        self._security = security_monitor or SecurityMonitor()
...
class SecurityMonitor:
    ...
    def __init__(
        ...
        self._rate_limit = SensorValidator(max_samples_per_sec=max_measurement_rate)
    ...
    def evaluate_measurement(
        ...
        alerts: list[SecurityAlert] = []
        filtered = self._rate_limit.validate({"offset": measurement.offset})
        if "offset" not in filtered:
            alerts.append(SecurityAlert(code="rate_limited", detail=f"{measurement.name} rate limited"))
        ...
        if measurement.name.lower().startswith("gps") and frame.ac_hum_hz is not None:
            if abs(measurement.offset) > self._divergence_threshold:
                alerts.append(SecurityAlert(code="gps_spoofing_suspected", detail="GPS offset spike detected"))
