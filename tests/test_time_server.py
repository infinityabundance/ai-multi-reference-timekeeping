...
def test_security_monitor_divergence_flag() -> None:
    safety = SafetyCase()
    for code in ["REFERENCE_DIVERGENCE", "GPS_SPOOFING", "AC_HUM_INJECTION", "SENSOR_FLOODING"]:
        safety.register(
            Hazard(
                code=code,
                description=code,
    ...
    monitor = SecurityMonitor(divergence_threshold=0.01, max_measurement_rate=0.0, safety_case=safety)
